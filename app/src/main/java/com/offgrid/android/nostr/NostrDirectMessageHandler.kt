package com.offgrid.android.nostr
...
import com.offgrid.android.model.PrivateMessagePacket
import com.offgrid.android.protocol.OffGridPacket
import com.offgrid.android.services.SeenMessageStore
import com.offgrid.android.ui.ChatState
import com.offgrid.android.ui.MeshDelegateHandler
import com.offgrid.android.ui.PrivateChatManager
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.offgrid.android.ui.DataManager
) {
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch(Dispatchers.Default) {
            try {
                ...
                if (!content.startsWith("OffGrid1:")) return@launch

                val base64Content = content.removePrefix("OffGrid1:")
                val packetData = base64URLDecode(base64Content) ?: return@launch
                val packet = OffGridPacket.fromBinaryData(packetData) ?: return@launch

                if (packet.type != com.offgrid.android.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch
                ...
                com.offgrid.android.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
    ...
    private suspend fun processNoisePayload(
        ...
        when (payload.type) {
            ...
            NoisePayloadType.FILE_TRANSFER -> {
                ...
                val file = OffGridFilePacket.decode(payload.data)
                if (file != null) {
                    val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                    val savedPath = com.offgrid.android.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = OffGridMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
