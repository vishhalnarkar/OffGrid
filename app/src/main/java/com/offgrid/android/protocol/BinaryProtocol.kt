package com.offgrid.android.protocol
...
data class OffGridPacket(
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = OffGridPacket(
            version = version,
            type = type,
            senderID = senderID,
            ...
            route = route,
            ttl = com.offgrid.android.util.AppConstants.SYNC_TTL_HOPS // Use fixed TTL=0 for signing to ensure relay compatibility
        )
        return BinaryProtocol.encode(unsignedPacket)
    }

    companion object {
        fun fromBinaryData(data: ByteArray): OffGridPacket? {
...
object BinaryProtocol {
    ...
    fun encode(packet: OffGridPacket): ByteArray? {
        try {
    ...
    fun decode(data: ByteArray): OffGridPacket? {
     ...
     */
    private fun decodeCore(raw: ByteArray): OffGridPacket? {
        try {
            ...
            if (payloadLength > com.offgrid.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH.toUInt()) {
                Log.w("BinaryProtocol", "Payload length ${payloadLength} exceeds maximum allowed (${com.offgrid.android.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH})")
                return null
            }
