...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
    ...
    suspend fun handleNoiseEncrypted(routed: RoutedPacket) {
        ...
        try {
            ...
            val noisePayload = com.offgrid.android.model.NoisePayload.decode(decryptedData)
            ...
            when (noisePayload.type) {
                com.offgrid.android.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                    // Decode TLV private message exactly like iOS
                    val privateMessage = com.offgrid.android.model.PrivateMessagePacket.decode(noisePayload.data)
                    if (privateMessage != null) {
                        ...
                        // Create OffGridMessage - preserve source packet timestamp
                ...
                com.offgrid.android.model.NoisePayloadType.FILE_TRANSFER -> {
                    // Handle encrypted file transfer; generate unique message ID
                    val file = com.offgrid.android.model.OffGridFilePacket.decode(noisePayload.data)
                    if (file != null) {
                        Log.d(TAG, "🔓 Decrypted encrypted file from $peerID: name='${file.fileName}', size=${file.fileSize}, mime='${file.mimeType}'")
                        val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                        val savedPath = com.offgrid.android.features.file.FileUtils.saveIncomingFile(appContext, file)
                        val message = OffGridMessage(
                            id = uniqueMsgId,
                            sender = delegate?.getPeerNickname(peerID) ?: "Unknown",
                            content = savedPath,
                            type = com.offgrid.android.features.file.FileUtils.messageTypeForMime(file.mimeType),
                            timestamp = java.util.Date(packet.timestamp.toLong()),
                            isRelay = false,
                            isPrivate = true,
                            recipientNickname = delegate?.getMyNickname(),
                ...
                com.offgrid.android.model.NoisePayloadType.DELIVERED -> {
                ...
                com.offgrid.android.model.NoisePayloadType.READ_RECEIPT -> {
                ...
                com.offgrid.android.model.NoisePayloadType.VERIFY_CHALLENGE -> {
                ...
                com.offgrid.android.model.NoisePayloadType.VERIFY_RESPONSE -> {
    ...
    private suspend fun sendDeliveryAck(messageID: String, senderPeerID: String) {
        try {
            ...
            val ackPayload = com.offgrid.android.model.NoisePayload(
                type = com.offgrid.android.model.NoisePayloadType.DELIVERED,
    ...
    suspend fun handleAnnounce(routed: RoutedPacket): Boolean {
        ...
        if (age > com.offgrid.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS) {
            Log.w(TAG, "Ignoring stale ANNOUNCE from ${peerID.take(8)} (age=${age}ms > ${com.offgrid.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS}ms)")
            return false
        }
        ...
        try {
            val neighborsOrNull = com.offgrid.android.services.meshgraph.GossipTLV.decodeNeighborsFromAnnouncementPayload(packet.payload)
        ...
        try {
            ...
            if (response != null) {
                ...
                val responsePacket = OffGridPacket(
                    ...
                    payload = response,
                    signature = null,
                    ttl = com.offgrid.android.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS
                )
    ...
    private suspend fun handleBroadcastMessage(routed: RoutedPacket) {
        ...
        try {
            ...
            val isFileTransfer = com.offgrid.android.protocol.MessageType.fromValue(packet.type) == com.offgrid.android.protocol.MessageType.FILE_TRANSFER
            val file = com.offgrid.android.model.OffGridFilePacket.decode(packet.payload)
            if (file != null) {
                if (isFileTransfer) {
     ...
     */
    private suspend fun handlePrivateMessage(packet: OffGridPacket, peerID: String) {
        try {
    ...
    private fun handleFavoriteNotificationFromMesh(content: String, fromPeerID: String) {
        try {
            ...
            if (noiseKey != null) {
                com.offgrid.android.favorites.FavoritesPersistenceService.shared.updatePeerFavoritedUs(noiseKey, isFavorite)
                if (npub != null) {
                    // Index by noise key and current mesh peerID for fast Nostr routing
                    com.offgrid.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKey(noiseKey, npub)
                    com.offgrid.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(fromPeerID, npub)
                }
                ...
                val sys = com.offgrid.android.model.OffGridMessage(
                    sender = "system",
...
interface MessageHandlerDelegate {
    ...
    // Packet operations
    fun sendPacket(packet: OffGridPacket)
    fun relayPacket(routed: RoutedPacket)
    fun getBroadcastRecipient(): ByteArray
    
    // Cryptographic operations
    fun verifySignature(packet: OffGridPacket, peerID: String): Boolean
    ...
    // Callbacks
    fun onMessageReceived(message: OffGridMessage)
