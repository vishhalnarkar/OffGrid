...
import com.offgrid.android.util.AppConstants
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.delay
import kotlinx.coroutines.launch
import java.util.*
import kotlinx.coroutines.Job
import com.offgrid.android.ui.debug.DebugSettingsManager
import com.offgrid.android.ui.debug.DebugScanResult
...
class BluetoothGattClientManager(
    ...
    fun start(): Boolean {
        ...
        try {
            if (!com.offgrid.android.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value) {
    ...
    fun onScanStateChanged(shouldScan: Boolean) {
        val enabled = try { com.offgrid.android.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true }
    ...
    private fun connectToDevice(device: BluetoothDevice, rssi: Int, peerID: String? = null) {
        ...
        val deviceAddress = device.address
        Log.i(TAG, "Connecting to OffGrid device: $deviceAddress (peerID: $peerID)")
        ...
        val gattCallback = object : BluetoothGattCallback() {
            ...
            override fun onCharacteristicChanged(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic) {
                ...
                val packet = OffGridPacket.fromBinaryData(value)
                if (packet != null) {
