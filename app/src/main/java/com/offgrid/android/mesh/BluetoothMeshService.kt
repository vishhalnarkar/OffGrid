package com.offgrid.android.mesh

import android.content.Context
import android.util.Log
import com.offgrid.android.crypto.EncryptionService
import com.offgrid.android.model.OffGridMessage
import com.offgrid.android.protocol.MessagePadding
import com.offgrid.android.model.RoutedPacket
import com.offgrid.android.model.IdentityAnnouncement
import com.offgrid.android.model.NoisePayload
import com.offgrid.android.model.NoisePayloadType
import com.offgrid.android.protocol.OffGridPacket
import com.offgrid.android.protocol.MessageType
import com.offgrid.android.protocol.SpecialRecipients
import com.offgrid.android.model.RequestSyncPacket
import com.offgrid.android.sync.GossipSyncManager
import com.offgrid.android.util.toHexString
import com.offgrid.android.services.VerificationService
...
class BluetoothMeshService(private val context: Context) {
    private val debugManager by lazy { try { com.offgrid.android.ui.debug.DebugSettingsManager.getInstance() } catch (e: Exception) { null } }
    
    companion object {
        private const val TAG = "BluetoothMeshService"
        private val MAX_TTL: UByte = com.offgrid.android.util.AppConstants.MESSAGE_TTL_HOPS
    }
    ...
    private val serviceNotificationManager = com.offgrid.android.ui.NotificationManager(
        context.applicationContext,
        androidx.core.app.NotificationManagerCompat.from(context.applicationContext),
        com.offgrid.android.util.NotificationIntervalManager()
    )
    ...
    init {
        ...
        gossipSyncManager = GossipSyncManager(
            ...
            configProvider = object : GossipSyncManager.ConfigProvider {
                override fun seenCapacity(): Int = try {
                    com.offgrid.android.ui.debug.DebugPreferenceManager.getSeenPacketCapacity(500)
                } catch (_: Exception) { 500 }

                override fun gcsMaxBytes(): Int = try {
                    com.offgrid.android.ui.debug.DebugPreferenceManager.getGcsMaxFilterBytes(400)
                } catch (_: Exception) { 400 }

                override fun gcsTargetFpr(): Double = try {
                    com.offgrid.android.ui.debug.DebugPreferenceManager.getGcsFprPercent(1.0) / 100.0
        ...
        gossipSyncManager.delegate = object : GossipSyncManager.Delegate {
            override fun sendPacket(packet: OffGridPacket) {
                connectionManager.broadcastPacket(RoutedPacket(packet))
            }
            override fun sendPacketToPeer(peerID: String, packet: OffGridPacket) {
                connectionManager.sendPacketToPeer(peerID, packet)
            }
            override fun signPacketForBroadcast(packet: OffGridPacket): OffGridPacket {
    ...
    private fun setupDelegates() {
        ...
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                ...
                try { com.offgrid.android.services.AppStateStore.setPeers(peerIDs) } catch (_: Exception) { }
            ...
            override fun onPeerRemoved(peerID: String) {
                ...
                try { com.offgrid.android.services.meshgraph.MeshGraphService.getInstance().removePeer(peerID) } catch (_: Exception) { }
        ...
        securityManager.delegate = object : SecurityManagerDelegate {
            ...
            override fun sendHandshakeResponse(peerID: String, response: ByteArray) {
                ...
                val responsePacket = OffGridPacket(
                    version = 1u,
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun verifySignature(packet: OffGridPacket, peerID: String): Boolean {
            ...
            override fun updatePeerIDBinding(newPeerID: String, nickname: String,
                ...
                try {
                    com.offgrid.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(publicKey)?.let { npub ->
                        com.offgrid.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(newPeerID, npub)
                    }
                } catch (_: Exception) { }
            ...
            // Callbacks
            override fun onMessageReceived(message: OffGridMessage) {
                // Always reflect into process-wide store so UI can hydrate after recreation
                try {
                    when {
                        message.isPrivate -> {
                            val peer = message.senderPeerID ?: ""
                            if (peer.isNotEmpty()) com.offgrid.android.services.AppStateStore.addPrivateMessage(peer, message)
                        }
                        message.channel != null -> {
                            com.offgrid.android.services.AppStateStore.addChannelMessage(message.channel!!, message)
                        }
                        else -> {
                            com.offgrid.android.services.AppStateStore.addPublicMessage(message)
                        }
                    }
                } catch (_: Exception) { }
                ...
                if (delegate == null && message.isPrivate) {
                    try {
                        ...
                        if (senderPeerID != null) {
                            ...
                            val preview = com.offgrid.android.ui.NotificationTextUtils.buildPrivateMessagePreview(message)
        ...
        packetProcessor.delegate = object : PacketProcessorDelegate {
            override fun validatePacketSecurity(packet: OffGridPacket, peerID: String): Boolean {
            ...
            override fun handleAnnounce(routed: RoutedPacket) {
                serviceScope.launch {
                    ...
                    if (deviceAddress != null && pid != null) {
                        ...
                        val isDirect = routed.packet.ttl == com.offgrid.android.util.AppConstants.MESSAGE_TTL_HOPS
            ...
            override fun handleFragment(packet: OffGridPacket): OffGridPacket? {
        ...
        override fun onPacketReceived(packet: OffGridPacket, peerID: String, device: android.bluetooth.BluetoothDevice?) {
            // Log incoming for debug graphs (do not double-count anywhere else)
            try {
                com.offgrid.android.ui.debug.DebugSettingsManager.getInstance().logIncoming(
                    packet = packet,
                    fromPeerID = peerID,
                    fromNickname = null,
            ...
            override fun onDeviceConnected(device: android.bluetooth.BluetoothDevice) {
                ...
                try {
                    ...
                    com.offgrid.android.ui.debug.DebugSettingsManager.getInstance()
     ...
     */
    fun sendFileBroadcast(file: com.offgrid.android.model.OffGridFilePacket) {
        try {
     ...
     */
    fun sendFilePrivate(recipientPeerID: String, file: com.offgrid.android.model.OffGridFilePacket) {
        try {
            ...
            serviceScope.launch {
                ...
                if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                    try {
                        ...
                        val noisePayload = com.offgrid.android.model.NoisePayload(
                            type = com.offgrid.android.model.NoisePayloadType.FILE_TRANSFER,
                            data = filePayload
                        )
                        ...
                        val packet = OffGridPacket(
                            ...
                            payload = encrypted,
                            signature = null,
                            ttl = com.offgrid.android.util.AppConstants.MESSAGE_TTL_HOPS
                        )
        ...
        serviceScope.launch {
            ...
            if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                try {
                    ...
                    val privateMessage = com.offgrid.android.model.PrivateMessagePacket(
                    ...
                    val messagePayload = com.offgrid.android.model.NoisePayload(
                        type = com.offgrid.android.model.NoisePayloadType.PRIVATE_MESSAGE,
                        data = tlvData
                    )
    ...
    fun sendReadReceipt(messageID: String, recipientPeerID: String, readerNickname: String) {
        serviceScope.launch {
            ...
            val geo = runCatching { com.offgrid.android.services.MessageRouter.tryGetInstance() }.getOrNull()
            val isGeoAlias = try {
                val map = com.offgrid.android.nostr.GeohashAliasRegistry.snapshot()
                map.containsKey(recipientPeerID)
            } catch (_: Exception) { false }
            if (isGeoAlias && geo != null) {
                geo.sendReadReceipt(com.offgrid.android.model.ReadReceipt(messageID), recipientPeerID)
                return@launch
            }

            try {
                // Avoid duplicate read receipts: check persistent store first
                val seenStore = try { com.offgrid.android.services.SeenMessageStore.getInstance(context.applicationContext) } catch (_: Exception) { null }
                ...
                val readReceiptPayload = com.offgrid.android.model.NoisePayload(
                    type = com.offgrid.android.model.NoisePayloadType.READ_RECEIPT,
                ...
                val packet = OffGridPacket(
                    ...
                    payload = encrypted,
                    signature = null,
                    ttl = com.offgrid.android.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS messageTTL
                )
    ...
    fun sendBroadcastAnnounce() {
        ...
        serviceScope.launch {
            val nickname = try { com.offgrid.android.services.NicknameProvider.getNickname(context, myPeerID) } catch (_: Exception) { myPeerID }
     ...
     */
    fun getSessionState(peerID: String): com.offgrid.android.noise.NoiseSession.NoiseSessionState {
     ...
     */
    private fun signPacketBeforeBroadcast(packet: OffGridPacket): OffGridPacket {
        return try {
            ...
            val withRoute = try {
                ...
                if (rec != null && !rec.contentEquals(SpecialRecipients.BROADCAST)) {
                    ...
                    val path = com.offgrid.android.services.meshgraph.RoutePlanner.shortestPath(myPeerID, dest)
...
interface BluetoothMeshDelegate {
    fun didReceiveMessage(message: OffGridMessage)
