...
class PrivateChatManager(
    ...
    private val unreadReceivedMessages = mutableMapOf<String, MutableList<OffGridMessage>>()
    ...
    fun startPrivateChat(peerID: String, meshService: BluetoothMeshService): Boolean {
        if (isPeerBlocked(peerID)) {
            ...
            val systemMessage = OffGridMessage(
                sender = "system",
    ...
    fun handleIncomingPrivateMessage(message: OffGridMessage) {
        handleIncomingPrivateMessage(message, suppressUnread = false)
    }

    fun handleIncomingPrivateMessage(message: OffGridMessage, suppressUnread: Boolean) {
    ...
    fun sendReadReceiptsForPeer(peerID: String, meshService: BluetoothMeshService) {
        ...
        val chats = try { state.getPrivateChatsValue() } catch (_: Exception) { emptyMap<String, List<OffGridMessage>>() }
    ...
    private fun consolidateNostrTempConversationIfNeeded(targetPeerID: String) {
        ...
        try {
            ...
            val npub = com.offgrid.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(noiseKeyBytes)
            if (npub != null) {
