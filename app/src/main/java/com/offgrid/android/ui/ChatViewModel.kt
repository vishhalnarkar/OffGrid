...
import com.offgrid.android.favorites.FavoritesPersistenceService
import kotlinx.coroutines.flow.StateFlow
import kotlinx.coroutines.flow.MutableStateFlow
import kotlinx.coroutines.flow.asStateFlow
import com.offgrid.android.mesh.BluetoothMeshDelegate
import com.offgrid.android.mesh.BluetoothMeshService
import com.offgrid.android.service.MeshServiceHolder
import com.offgrid.android.model.OffGridMessage
import com.offgrid.android.model.OffGridMessageType
import com.offgrid.android.nostr.NostrIdentityBridge
import com.offgrid.android.protocol.OffGridPacket


import kotlinx.coroutines.launch
import com.offgrid.android.util.NotificationIntervalManager
import kotlinx.coroutines.delay
import kotlinx.coroutines.launch
import java.util.Date
import kotlin.random.Random
import com.offgrid.android.services.VerificationService
import com.offgrid.android.identity.SecureIdentityStateManager
import com.offgrid.android.noise.NoiseSession
import com.offgrid.android.nostr.GeohashAliasRegistry
import com.offgrid.android.util.dataFromHexString
import com.offgrid.android.util.hexEncodedString
import java.security.MessageDigest

/**
 * Refactored ChatViewModel - Main coordinator for OffGrid functionality
...
class ChatViewModel(
    ...
    val messages: StateFlow<List<OffGridMessage>> = state.messages
    ...
    val privateChats: StateFlow<Map<String, List<OffGridMessage>>> = state.privateChats
    ...
    val channelMessages: StateFlow<Map<String, List<OffGridMessage>>> = state.channelMessages
    ...
    val selectedLocationChannel: StateFlow<com.offgrid.android.geohash.ChannelID?> = state.selectedLocationChannel
    ...
    init {
        ...
        viewModelScope.launch {
            try { com.offgrid.android.services.AppStateStore.peers.collect { peers ->
                state.setConnectedPeers(peers)
                state.setIsConnected(peers.isNotEmpty())
            } } catch (_: Exception) { }
        }
        viewModelScope.launch {
            try { com.offgrid.android.services.AppStateStore.publicMessages.collect { msgs ->
                // Source of truth is AppStateStore; replace to avoid duplicate keys in LazyColumn
                state.setMessages(msgs)
            } } catch (_: Exception) { }
        }
        viewModelScope.launch {
            try { com.offgrid.android.services.AppStateStore.privateMessages.collect { byPeer ->
                ...
                try {
                    val seen = com.offgrid.android.services.SeenMessageStore.getInstance(getApplication())
        ...
        viewModelScope.launch {
            try { com.offgrid.android.services.AppStateStore.channelMessages.collect { byChannel ->
                // Replace with store snapshot
        ...
        viewModelScope.launch {
            com.offgrid.android.mesh.TransferProgressManager.events.collect { evt ->
    ...
    private fun loadAndInitialize() {
        ...
        viewModelScope.launch {
            com.offgrid.android.ui.debug.DebugSettingsManager.getInstance().debugMessages.collect { msgs ->
                if (com.offgrid.android.ui.debug.DebugSettingsManager.getInstance().verboseLoggingEnabled.value) {
                    ...
                    if (selectedLocation is com.offgrid.android.geohash.ChannelID.Mesh) {
        ...
        com.offgrid.android.favorites.FavoritesPersistenceService.initialize(getApplication())
        ...
        try {
            val nostrTransport = com.offgrid.android.nostr.NostrTransport.getInstance(getApplication())
    ...
    private fun ensureGeohashDMSubscriptionIfNeeded(convKey: String) {
        try {
            ...
            val repo = repoField.get(geohashViewModel) as com.offgrid.android.nostr.GeohashRepository
            ...
            if (!gh.isNullOrEmpty()) {
                ...
                val subMgr = subMgrField.get(geohashViewModel) as com.offgrid.android.nostr.NostrSubscriptionManager
                val identity = com.offgrid.android.nostr.NostrIdentityBridge.deriveIdentity(gh, getApplication())
                val subId = "geo-dm-$gh"
                ...
                if (currentId != subId) {
                    ...
                    subMgr.subscribeGiftWraps(
                        ...
                        handler = { event ->
                            ...
                            val dmHandler = dmHandlerField.get(geohashViewModel) as com.offgrid.android.nostr.NostrDirectMessageHandler
    ...
    fun openLatestUnreadPrivateChat() {
        try {
            ...
            val openPeer: String = if (targetKey.startsWith("nostr_")) {
                ...
                val canonical = com.offgrid.android.services.ConversationAliasResolver.resolveCanonicalPeerID(
                    ...
                    nostrPubHexForAlias = { alias -> com.offgrid.android.nostr.GeohashAliasRegistry.get(alias) },
                    findNoiseKeyForNostr = { key -> com.offgrid.android.favorites.FavoritesPersistenceService.shared.findNoiseKey(key) }
                )
                canonical ?: targetKey
            }
    ...
    fun sendMessage(content: String) {
        ...
        if (content.startsWith("/")) {
            ...
            commandProcessor.processCommand(content, meshService, meshService.myPeerID, { messageContent, mentions, channel ->
                if (selectedLocationForCommand is com.offgrid.android.geohash.ChannelID.Location) {
        ...
        if (selectedPeer != null) {
            ...
            selectedPeer = com.offgrid.android.services.ConversationAliasResolver.resolveCanonicalPeerID(
            ...
            privateChatManager.sendPrivateMessage(
                ...
                val router = com.offgrid.android.services.MessageRouter.getInstance(getApplication(), meshService)
                router.sendPrivate(messageContent, peerID, recipientNicknameParam, messageId)
            }
        } else {
            // Check if we're in a location channel
            val selectedLocationChannel = state.selectedLocationChannel.value
            if (selectedLocationChannel is com.offgrid.android.geohash.ChannelID.Location) {
    ...
    fun toggleFavorite(peerID: String) {
        ...
        try {
            ...
            if (peerInfo?.noisePublicKey != null) {
                ...
                if (peerID.length == 64 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                    try {
                        ...
                        val rel = com.offgrid.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey!!)
            ...
            if (noiseKey != null) {
                ...
                val identityManager = com.offgrid.android.identity.SecureIdentityStateManager(getApplication())
                ...
                com.offgrid.android.favorites.FavoritesPersistenceService.shared.updateFavoriteStatus(
                ...
                try {
                    val myNostr = com.offgrid.android.nostr.NostrIdentityBridge.getCurrentNostrIdentity(getApplication())
    ...
    private fun updateReactiveStates() {
        ...
        sessionStates.forEach { (peerID, newState) ->
            ...
            if (old != "established" && newState == "established") {
                com.offgrid.android.services.MessageRouter
    ...
    override fun didReceiveMessage(message: OffGridMessage) {
    ...
    fun panicClearAllData() {
        ...
        try {
            com.offgrid.android.services.SeenMessageStore.getInstance(getApplication()).clear()
        } catch (_: Exception) { }
        ...
        com.offgrid.android.features.file.FileUtils.clearAllMedia(getApplication())
        ...
        try {
            ...
            try {
                val store = com.offgrid.android.geohash.GeohashBookmarksStore.getInstance(getApplication())
                store.clearAll()
            } catch (_: Exception) { }
    ...
    fun selectLocationChannel(channel: com.offgrid.android.geohash.ChannelID) {
