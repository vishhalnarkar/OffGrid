...
import com.offgrid.android.model.OffGridFilePacket
...
class MediaSendingManager(
    ...
    companion object {
        ...
        private const val MAX_FILE_SIZE = com.offgrid.android.util.AppConstants.Media.MAX_FILE_SIZE_BYTES // 50MB limit
    }
    ...
    fun sendVoiceNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            val filePacket = OffGridFilePacket(
                fileName = file.name,
            ...
            if (toPeerIDOrNull != null) {
                sendPrivateFile(toPeerIDOrNull, filePacket, filePath, OffGridMessageType.Audio)
            } else {
                sendPublicFile(channelOrNull, filePacket, filePath, OffGridMessageType.Audio)
            }
        } catch (e: Exception) {
    ...
    fun sendImageNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            if (toPeerIDOrNull != null) {
                sendPrivateFile(toPeerIDOrNull, filePacket, filePath, OffGridMessageType.Image)
            } else {
                sendPublicFile(channelOrNull, filePacket, filePath, OffGridMessageType.Image)
            }
        } catch (e: Exception) {
    ...
    fun sendFileNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            val mimeType = try { 
                com.offgrid.android.features.file.FileUtils.getMimeTypeFromExtension(file.name) 
            } catch (_: Exception) { 
            ...
            val messageType = when {
                mimeType.lowercase().startsWith("image/") -> OffGridMessageType.Image
                mimeType.lowercase().startsWith("audio/") -> OffGridMessageType.Audio
                else -> OffGridMessageType.File
            }
    ...
    private fun sendPrivateFile(
        toPeerID: String,
        filePacket: OffGridFilePacket,
        filePath: String,
        messageType: OffGridMessageType
    ) {
        ...
        messageManager.updateMessageDeliveryStatus(
            msg.id,
            com.offgrid.android.model.DeliveryStatus.PartiallyDelivered(0, 100)
        )
     ...
     */
    fun handleTransferProgressEvent(evt: com.offgrid.android.mesh.TransferProgressEvent) {
        val msgId = synchronized(transferMessageMap) { transferMessageMap[evt.transferId] }
        if (msgId != null) {
            if (evt.completed) {
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.offgrid.android.model.DeliveryStatus.Delivered(to = "mesh", at = java.util.Date())
                )
                synchronized(transferMessageMap) {
                ...
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.offgrid.android.model.DeliveryStatus.PartiallyDelivered(evt.sent, evt.total)
                )
            }
        }
    }
